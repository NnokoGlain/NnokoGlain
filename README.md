# Glain Elambo Nnoko
### IT Support | QA & User Insights | Project Coordination

📍 Bahrain | Open to Remote & International Roles  
📧 nnokoglain@gmail.com  
🔗 LinkedIn: https://www.linkedin.com/in/glainnnoko98

---

## 👋 About Me

I am an IT professional with over **2 years of experience** in technical support and project coordination, supporting users in remote and onsite environments. I have hands-on experience with **Windows, Linux, Microsoft 365, Active Directory**, and familiarity with **AWS cloud environments**, alongside strong troubleshooting and documentation skills.

In addition, I bring a project-focused mindset, supported by my **PMP certification** and experience coordinating tasks, timelines, and documentation across teams. I am currently pursuing a **B.S. in Information Technology** at Ensign College (USA), anticipated 2026.

---

## 🧰 Technical Skills

### IT Support & Systems
- End-user & help desk support (remote and onsite)
- Hardware & software troubleshooting
- Windows & Linux administration
- Active Directory user & group management
- Microsoft 365 (Outlook, Teams, OneDrive)

### QA & User Insights
- Manual & exploratory testing
- Bug identification & documentation
- Re-testing fixes
- User-focused analysis

### Project Coordination
- Task & timeline coordination
- Documentation & reporting
- Stakeholder communication

---

## ☁️ Cloud (AWS) – Foundational Experience
- Familiar with AWS cloud environment
- Basic EC2 & IAM understanding
- VPC & security group awareness
- AWS Management Console navigation

---

## 🛠 Tools
Jira | Asana | Slack | Microsoft 365  
VMware | VirtualBox | AWS Console

---

## 💼 Experience

### Customer Support Specialist  
**Morning Star Co. W.L.L., Bahrain**  
*Aug 2023 – Present*
- Remote and onsite technical support
- Hardware, software, and network troubleshooting
- Documentation and escalation handling

### Technical Support Assistant  
**Al Muheet Institute, Bahrain**  
*Mar 2022 – Feb 2023*
- Created and managed student accounts
- Supported OneDrive file sharing
- Installed and updated software
- Assisted users with system access

---

## 🎯 Roles I'm a Strong Fit For
- IT Support Specialist
- Help Desk Technician (Tier 1–2)
- Technical Support Engineer (Junior–Mid)
- IT Operations Support
- QA / Bug Tester
- User Insights Specialist
- IT Project Coordinator / Junior Project Manager
- Customer Technology Support (SaaS)
- Cloud Support Associate (AWS – Entry Level)
- Junior Systems Administrator

---

## EDUCATION
B.Sc. Information Technology (USA) – Ensign College Anticipated June. 2026

## Certifications
- Google IT Support - (2024)
- IT Professional - Ensign College, USA (2025)
- Technical Support Engineer - Ensign College (2024)
- TestOut PC Pro | TestOut Network Pro | TestOut Client Pro certifications

---

## 📄 Resume
- IT Support CV / Technical Support
- IT Project Coordinator CV
Please feel free to reach out via email or linkedIn.

---

## 📬 Contact
📧 Email: nnokoglain@gmail.com  
🔗 LinkedIn: https://www.linkedin.com/in/glainnnoko98

